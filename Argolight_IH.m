function Argolight_IH(I1,test_I,varargin)
% Author: Joshua Yeh
...
for dum=1:size(centroids,1)
    c=[floor(centroids(dum,1)-4) ceil(centroids(dum,1)+4)];
    r=[floor(centroids(dum,2)-4) ceil(centroids(dum,2)+4)];
    if c(1)<1;   c(1)=1;     end
    if r(1)<1;   r(1)=1;     end
    ...
    [M,jj]=max(subimage(:));
    [r1,c1]=ind2sub(size(subimage),jj);
...
opts = fitoptions( 'Method', 'LowessFit',...
    'normalize','on','robust','bisquare','span',0.05);
...
% subplot 2
surf(f1.s2,test_I);
plot(f1.s2,centroids(:,1),centroids(:,2),'rx','displayname','grid pattern');
