...
% EDITED by Joshua Yeh (josh.yeh@espci.fr)
% Updated on 18/06/04
...
parser.addParameter('xEdges',[]);
parser.addParameter('yEdges',[]);
parser.addParameter('zdata',[]);
...
if isempty(numBins)
    ...
    if isempty(yEdges)||isempty(xEdges)
...
theta = (1/6 : 1/6 : 1).* 2.*pi;

% Calculate vertex coordinates
XVertices = bsxfun(@plus, XCenters, rx .* cos(theta)).';
YVertices = bsxfun(@plus, YCenters, ry .* sin(theta)).';
